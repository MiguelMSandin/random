...
if args.verbose:
	print("  Selecting sequences from file:", args.file_in)
...
if args.verbose:
	if args.action == "remove" or args.action == "r":
		print("  File with removed sequences exported to:", outFile)
	if args.action == "keep" or args.action == "k":
		print("  File with selected sequences exported to:", outFile)
	if args.listSeq is not None:
		missing = set()
		for l in listSeq:
			if l not in seqsid:
